...
public class JRiot {
     ...
     * Get Rune of id.
     *
     * @param runeID Ids of summoners.
     * @return rune
     * @throws JRiotException
     */
    public Rune getRunes(int runeID) throws JRiotException{
        ApiCaller caller = new ApiCaller();
        String response = caller.request("https://" + region + ".api.pvp.net/api/lol/static-data/" + region + "/v1.2/rune/" + runeID + "?runeData=all&api_key=" + apiKey);
    Rune rune = gson.fromJson(response, new TypeToken<Rune>(){}.getType());
    return rune;
    }
